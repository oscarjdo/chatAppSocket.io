...
import { setNotiState } from "../../app/notiSlice.js";
import { setFriendListLoad } from "../../app/allLoadsSlice.js";
...
import SetIcon from "./friendList/SetIcon.jsx";
...
function friendList() {
  ...
  const handleClickPlus = (friend) => {
    ...
    dispatch(
      setNotiState({
        active: true,
        text: `You have sent a friend request to ${friend.friend}`,
        type: "WARNING",
      })
    );
  };
  ...
  const handleClickCancel = (friend) => {
    ...
    dispatch(
      setNotiState({
        active: true,
        text: `You have canceled ${friend.friend}'s friend request.`,
        type: "WARNING",
      })
    );
  };
                    ...
                    <div className="text-ctn">
                      ...
                      <div className="p-ctn">
                        ...
                        <p>
                            ...
                            <span className="groupSenderText">
                              {item.lastMessage.sender != userState.id
                                ? item.members
                                    .filter(
                                      (it) =>
                                        it.userId == item.lastMessage.sender
                                    )
                                    .map((it) => it.username)
                                : "You"}
                              :&nbsp;
                          ...
                          {itemDate ? (
                            <>
                              {item.lastMessage.mimetype &&
                              item.lastMessage.is_show
                                ? setFileType()
                                : null}
                              {generateText()}
                            </>
